...
interface PrescriptionListProps {
  ...
  onEditPrescription: (prescription: PrescriptionWithDetails) => void;
...
export const PrescriptionList: React.FC<PrescriptionListProps> = ({
  ...
  const groupPrescriptionsByDate = (prescriptions: PrescriptionWithDetails[]) => {
    const grouped: { [key: string]: PrescriptionWithDetails[] } = {};
    
    prescriptions.forEach(prescription => {
      const date = prescription.date_emission;
      if (!grouped[date]) {
        grouped[date] = [];
      }
  ...
  const groupedPrescriptions = groupPrescriptionsByDate(prescriptions);
  const sortedDates = Object.keys(groupedPrescriptions).sort().reverse();
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-purple-600">
                {prescriptions.filter(p => {
                  const prescriptionDate = new Date(p.date_emission);
            ...
            <div key={date} className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
              <div className="bg-gray-50 px-6 py-4 border-b border-gray-100">
                ...
                <p className="text-sm text-gray-600">
                  {groupedPrescriptions[date].length} ordonnance(s)
                </p>
              </div>
                      ...
                      <div className="flex-1 space-y-3">
                        ...
                        <div className="flex items-center space-x-4">
                          ...
                          <div className="text-sm text-gray-600">
                            par {prescription.professionnel_prenom} {prescription.professionnel_nom} ({prescription.professionnel_role})
                          </div>
                          
                          {prescription.consultation_diagnostic && (
                            <div className="bg-blue-100 text-blue-800 px-2 py-1 rounded-full text-xs">
                              Consultation: {prescription.consultation_diagnostic}
                            </div>
                          )}
                        </div>
                        ...
                        <div>
                          <div className="flex items-center space-x-1 mb-2">
                            ...
                            <span className="text-sm font-medium text-gray-700">
                              Médicaments ({prescription.liste_medicaments.length})
                            </span>
                          </div>
                          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
                            {prescription.liste_medicaments.slice(0, 3).map((medication, index) => (
                              <div key={index} className="bg-gray-50 p-2 rounded text-xs">
                                <span className="font-medium">{medication.nom}</span> {medication.dosage}
                                <br />
                                <span className="text-gray-600">{medication.frequence} - {medication.duree}</span>
                              </div>
                            ))}
                            {prescription.liste_medicaments.length > 3 && (
                              <div className="bg-gray-100 p-2 rounded text-xs flex items-center justify-center text-gray-600">
                                +{prescription.liste_medicaments.length - 3} autres
                              </div>
                            )}
                          </div>
                        </div>
                        ...
                        {prescription.instructions_generales && (
                          <div>
                            ...
                            <p className="text-sm text-gray-600 line-clamp-2">{prescription.instructions_generales}</p>
                          </div>
                        )}
                      </div>
